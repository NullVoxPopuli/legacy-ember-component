  ...
  implements PropertyDidChange
{
  ...
  */
  ...
  init(properties?: object) {
    ...
    if (eventDispatcher) {
      let lazyEventsProcessedForComponentClass =
        lazyEventsProcessed.get(eventDispatcher);
      ...
      if (!lazyEventsProcessedForComponentClass.has(proto)) {
        ...
        lazyEvents.forEach((mappedEventName, event) => {
          if (
            ...
            typeof (this as any)[mappedEventName] === 'function'
          ) {
    ...
    assert(
      ...
      descriptorForProperty(this, 'classNames') === undefined &&
  ...
  get _dispatcher(): EventDispatcher | null {
    if (this.__dispatcher === undefined) {
      ...
      if ((owner.lookup('-environment:main') as Environment).isInteractive) {
        const dispatcher = owner.lookup('event_dispatcher:main');
  ...
  readDOMAttr(name: string) {
    ...
    const { type, normalized } = normalizeProperty(
  ...
  */
  ...
  _transitionTo(this: Component, state: keyof typeof ViewStates) {
    const priorState = this._currentState;
    const currentState = (this._currentState = this._states[state]);
